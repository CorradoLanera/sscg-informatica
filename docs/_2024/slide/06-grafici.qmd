...
- Usando il dataset appena importato, create un grafico a piacere utilizzando `{ggplot2}`.
- usate almeno tre variabili del dataset per il grafico (x, y, colore, ...).
- a piacimento aggiungete titoli, etichette, didascalie e un tema.
- salvate il grafico in formato PDF e PNG utilizzando `ggsave`. (opzionale, salvarlo in una cartella `output` del progetto).
